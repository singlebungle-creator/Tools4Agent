module.exports={
	...
	ticketCalculator: `
			<script src="/lib/templateFunctions.js"></script>
			<table>
				<tr>
					<th scope='col'>1천</th>
	...
	<script src="/lib/calendar.js"></script>`;
		for (var title in _fileDict) {
			tmp+=`<p hidden id='${title}'>${_fileDict[title]}</p>`;
		}
		tmp+=`
	<h1 style="margin:0px" id='currentYear'></h1>
	<div><button style="display:inline-block" onclick="prevMonth()">◁</button><p style="display:inline-block" id="currentMonth" name=''></p><p hidden id="currentFirst"></p><button style="display:inline-block" onclick="nextMonth()">▷</button></div>
	<div id="calendar"></div>
	<script>
	document.getElementById('currentYear').innerText=currentYear;
	document.getElementById('currentMonth').name=today.getMonth();
	document.getElementById('currentMonth').innerText=monthList[today.getMonth()][0];
	document.getElementById('currentFirst').innerText=getFirst();
